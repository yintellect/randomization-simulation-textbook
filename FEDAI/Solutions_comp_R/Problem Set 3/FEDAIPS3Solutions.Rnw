...
library(foreign)
library(arm)
setwd("~/Documents/Dropbox/Columbia/spring 2014/experiments2014/FEDAI AEC Data and R Programs/Chapter 3/")
hajj <- read.dta("Clingingsmith subset.dta")
# set global chunk options
opts_chunk$set(fig.path='figure/PS3-', fig.align='center', fig.show='hold', out.width = '4in', out.height='4in', tidy=FALSE)
...
\section*{Question 3}
Using the equation $Y_i (1) = Y_i (0) + \tau_i$, show that when we assume that treatment effects are the same for all subjects, $Var(Y)_i (0))= Var(Y)_i (1))$ and the correlation between $Y_i (0)$ and $Y_i (1)$ is 1.0.[5 points]
...
The estimated ATE is \Sexpr{ate}.  The number of simulated ATEs under the sharp null hypothesis of no effect that were large was \Sexpr{sum(distout>ate)}, corresponding to a $p$-value of \Sexpr{p.value.onesided}.  The number of simulated ATEs under the sharp null hypothesis of no effect that were large in absolute value was  was \Sexpr{sum(abs(distout)>ate)}, corresponding to a $p$-value of \Sexpr{p.value.twosided}.
...
The estimated ATE in Texas is \Sexpr{round(ate_texas,3)}.  In Arkansas, the estimated ATE is \Sexpr{round(ate_arkansas,3)}.
...
The estimated se in Texas is \Sexpr{round(se_texas,3)}.  In Arkansas, the estimated se is \Sexpr{round(se_arkansas,3)}.
...
The overall ATE, \Sexpr{round(ate_overall,3)} is the weighted average of the two separate ATEs, where the weights are the shares of overall $N$ in each state.
...
The overall standard error is (\Sexpr{round(se_overall,3)}).
...
We conducted 10,000 random assignments, and for each we calculated the F-statistic of a regression of treatment assignment on pre-experimental bets (controlling for blocks).  The observed F-statistic for the actual experiment is larger than \Sexpr{sum(Fstatstore >= Fstat)} of the simulated experiments, implying a p-value of \Sexpr{round(p.value,3)}.
...
ATE
@
The average treatment group change was \$\Sexpr{round(change_treatment,2)}, as opposed to an average change of \$\Sexpr{round(change_control,2)} in the control group.  Therefore, the estimated ATE is \$\Sexpr{round(ATE,2)}.
...
mean(pair_diffs)
@
The average difference between treatment and control outcomes for each pair is also \Sexpr{round(mean(pair_diffs),2)}.
...
\section*{Question 11}
<<Q11setup, include=FALSE, cache=FALSE>>=
rm(list=ls())
@
...
Assuming that 4 out of 7 clusters are assigned to treatment, the standard error of the ATE will be \Sexpr{round(se_ate,2)}.
