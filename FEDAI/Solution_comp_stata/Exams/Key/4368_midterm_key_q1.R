...
q1c.out[x,1] <- n.clust
q1c.out[x,2] <- itt
q1c.out[x,3] <- dispout$two.tailed.p.value.abs
q1c.out[x,4] <- dispout$greater.p.value
q1c.out[x,5] <- dispout$lesser.p.value

Ys <- genouts(Y, Z, ate=itt)
distout <- gendist(Ys,perms,prob=probs, HT=FALSE)
dispout <- dispdist(distout, itt, display.plot=FALSE)
q1c.out[x,6] <- dispout$quantile[1] - (sqrt((n.clust-1)/(n.clust-2))/2)
q1c.out[x,7] <- dispout$quantile[2] + (sqrt((n.clust-1)/(n.clust-2))/2)
...
q1c.out[x,8] <- itt
q1c.out[x,9] <- dispout$two.tailed.p.value.abs
q1c.out[x,10] <- dispout$greater.p.value
q1c.out[x,11] <- dispout$lesser.p.value

Ys <- genouts(Y, Z, ate=itt)
distout <- gendist(Ys,X=X,perms,prob=probs, HT=FALSE)
dispout <- dispdist(distout, itt, display.plot=FALSE)
q1c.out[x,12] <- dispout$quantile[1] - (sqrt((n.clust-1)/(n.clust-2))/2)
q1c.out[x,13] <- dispout$quantile[2] + (sqrt((n.clust-1)/(n.clust-2))/2)
...
distout <- gendist(Ys,perms,prob=probs, HT=TRUE)
dispout <- dispdist(distout, itt, display.plot=FALSE)
q1e.out[x,5] <- dispout$quantile[1] - (sqrt((n.clust-1)/(n.clust-2))/2)
q1e.out[x,6] <- dispout$quantile[2] + (sqrt((n.clust-1)/(n.clust-2))/2)
...
distout <- gendist(Ys,X=X,perms,prob=probs, HT=TRUE)
dispout <- dispdist(distout, itt, display.plot=FALSE)
q1e.out[x,11] <- dispout$quantile[1] - (sqrt((n.clust-1)/(n.clust-2))/2)
q1e.out[x,12] <- dispout$quantile[2] + (sqrt((n.clust-1)/(n.clust-2))/2)
...
q1h.out[x,5] <- ci.lower.cace - (sqrt((n.clust-1)/(n.clust-2))/2)
q1h.out[x,6] <- ci.upper.cace + (sqrt((n.clust-1)/(n.clust-2))/2)
...
q1h.out[x,11] <- ci.lower.cace - (sqrt((n.clust-1)/(n.clust-2))/2)
q1h.out[x,12] <- ci.upper.cace + (sqrt((n.clust-1)/(n.clust-2))/2)
...
for(y in 1:nrow(stinfo)){
	...
	setwd("/Users/al/Dropbox/Teaching/S13 Experiments/Midterm/Key/")
	write.csv(q1b.table.out[,,y], paste("q1b_",as.character(stinfo$uni[y]),".csv",sep=""))
...
setwd("/Users/al/Dropbox/Teaching/S13 Experiments/Midterm/Key/")
write.csv(q1c.out, "q1c.csv")
write.csv(q1e.out, "q1e.csv")
write.csv(q1g.out, "q1g.csv")
write.csv(q1h.out, "q1h.csv")
write.csv(q1i.out, "q1i.csv")
