...
opts_chunk$set(fig.path='figure/PS12-', fig.align='center', fig.show='hold', out.width = '4in', out.height='4in', tidy=FALSE)
...
\section*{Question 1}
Stewart Page performed an audit study to measure the extent to which gay people en- counter discrimination in the rental housing market.\footnote{Page 1998.} Answer the following questions, which direct your attention to specific page numbers in the original article.
...
A design that addresses both possible explanations will need to randomize increased incarceration at both levels. Suppose there are 500 municipalities and a list of (not currently incarcerated) criminals is available for all 500. First, we select a random 250 municipalities to recieve treatment. In the treatment group municipalities, we randomly select half of the criminals to be locked up for 1 year.
...
We can further compare the criminial activity of treated and untreated criminials in the year after the crackdown: If treated criminals are less likely to reoffend, then we have evidence for the second explanation. 
...
A partial solution to this problem would be to measure outcomes other than whether or not the applicant is called for an interview or actually hired. 

Suppose 1,000 hiring managers were enrolled in an online panel that measures their attitudes towards blacks. 500 of these managers would be treated (in the normal course of their business) with a white applicant and the other 500 with a black applicant. We then re-measure their attitudes towards blacks. If the "black" group has more negative racial attitudes than the "white" group, we can conclude that getting a resume with a black name *primed* this negative affect. This design answers a slightly different question from the one posed; answers to that question may be fundamentally unidentified.
...
\begin{enumerate}[a)]
\item What are some important potential sources of asymmetry in outcome measure-
ment? Consider among other things how the test is administered, who proctors the test, who grades the test, the mixture of students in the room for a testing session, and whether the administration and grading is blinded to the subject's group status.\\
Answer:\\
...
<<echo=FALSE>>=
library(foreign)
howell <- read.dta("/Users/alex/Documents/Dropbox/Columbia/Spring 2014/Experiments2014/FEDAI AEC Data and R Programs/Chapter 12/Howell and Peterson replication subset for Chapter 12 problem set.dta")
@
...
Estimate the standard error for the difference-in-means estimator using the formula in equation (3.6), letting the number of observations assigned to treatment be nt and the number of observations assigned to control be $n_c$. The standard error may be written:
...
Set up a constrained maximation problem by defining the Lagrangian equation (Dixit 1990)
\begin{equation*}
...
\item Set the partial derivative with respect to nt equal to the partial derivative with respect to $n_c$. Manipulate the resulting equation to show that
\begin{equation*}
